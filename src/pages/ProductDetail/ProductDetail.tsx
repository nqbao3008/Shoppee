...
export default function ProductDetail() {
  ...
  const [currentImageListIndex, setCurrentImageListIndex] = useState([0, 5])
  ...
  const handleActiveImage = (image: string) => setCurrentImage(image)

  const handleNext = () => {
    if (currentImageListIndex[1] < product.images.length) {
      setCurrentImageListIndex((prev) => [prev[0] + 1, prev[1] + 1])
    }
  }

  const handleBack = () => {
    if (currentImageListIndex[0] > 0) {
      setCurrentImageListIndex((prev) => [prev[0] - 1, prev[1] - 1])
    }
  }
  ...
  const handleZoomIn = (event: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
    ...
    // Cách 1: Lấy offsetX, offsetY đơn giản khi chúng ta đã xử lý được bubble event (pointer-events-none -> dùng css này cho child elements)
          ...
          <div className='grid grid-cols-12 gap-9'>
            <div className='col-span-5'>
              ...
              <div className='relative mt-4 grid grid-cols-5 gap-1'>
                ...
                {product.images.slice(...currentImageListIndex).map((img) => {
                  const isActive = img === currentImage
                  return (
                    <div className='relative w-full pt-[100%]' key={img} onMouseEnter={() => handleActiveImage(img)}>
                      <img
                        src={img}
                        alt={product.name}
